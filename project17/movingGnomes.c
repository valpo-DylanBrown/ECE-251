...
// keeps track of total elapsed time
int RunningETime = 0;
double RSeed = 0;
...
void renderScene(void) {
     ...
     // If 1 second has passed, get new seed:
     RunningETime += etime;
     if (RunningETime > 1000){
       RunningETime -= 1000;
       RSeed = time(NULL);
     }

     srand(RSeed);   //Using the same seed every time will generate the same locations.
     

     for (counter=1; counter<=100; counter++){
       drawGnome(spacing*rand()/32767, spacing*rand()/32767);
       // USE THIS LINE FOR MAC or LINUX:
       //drawGnome(spacing*rand()/RAND_MAX, spacing*rand()/RAND_MAX);
     }
     glutSwapBuffers();
} 
